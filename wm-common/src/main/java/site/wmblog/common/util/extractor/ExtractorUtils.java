...
public abstract class ExtractorUtils {
    ...
    public static <E, K, V> Map<K, V> extractAsMap(Collection<E> entries, Extractor<E, Pair<K, V>> extractor) {
        ...
        Map<K, V> result = Maps.newHashMapWithExpectedSize(entries.size());
        for (E e : entries) {
    ...
    public static <K, V> Map<K, V> extractArrayAsMap(Pair<K, V>... pairs) {
        if (ArrayUtils.isEmpty(pairs)) {
            return Collections.emptyMap();
        }
        Map<K, V> result = Maps.newHashMapWithExpectedSize(pairs.length);
